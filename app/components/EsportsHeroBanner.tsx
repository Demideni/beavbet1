...
/**
 * Standalone promo banner (same style as crypto hero).
 * Put your esports art at: /public/banners/hero-esports.png
 */
export function EsportsHeroBanner() {
  const b = banners.find((x) => x.id === "esports") ?? banners[0];
  ...
  return (
    <section className="w-full rounded-3xl overflow-hidden card-glass gradient-hero">
      <div className="relative min-h-[220px] sm:min-h-[240px]">
        <Image src={b.art} alt={b.title} fill className="object-cover opacity-90" priority={false} />
        ...
        <div className="relative z-10 p-5 sm:p-6 flex flex-col justify-between min-h-[220px] sm:min-h-[240px]">
          <div className="max-w-[620px]">
            <div className="text-accent font-extrabold tracking-tight text-3xl sm:text-4xl leading-none">{b.title}</div>
            <div className="mt-2 text-white font-extrabold tracking-tight text-3xl sm:text-4xl leading-none">{b.subtitle}</div>
